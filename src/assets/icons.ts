 ...
 * 
 * All icons are 16x16 by default, using inline SVG for:
 ...
 * - ChatGPT: Official OpenAI branding
 * - Gemini: Official Google branding
 * 
 * @module icons
 * @version 1.0.0
 */
...
/**
 * Icon collection
 * All SVG icons as string templates
 */
export const Icons = {
    // ============================================
    ...
    /**
     * Claude official icon
     * Source: Anthropic official branding
     * Color: Warm brown/amber tone
     */
    claude: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M7.245 2.962c.598-.797 1.472-.962 2.07-.962.598 0 1.472.165 2.07.962a3.235 3.235 0 0 1 0 3.576c-.598.797-1.472.962-2.07.962-.598 0-1.472-.165-2.07-.962a3.235 3.235 0 0 1 0-3.576z" fill="currentColor" fill-opacity="0.9"/>
        <path d="M5.5 5c0-1.657 1.343-3 3-3s3 1.343 3 3-1.343 3-3 3-3-1.343-3-3z" fill="currentColor" fill-opacity="0.6"/>
        <circle cx="8.5" cy="8.5" r="1.5" fill="currentColor" fill-opacity="0.4"/>
    </svg>`,
 ...
 * @example
 * ```typescript
 * const icon = getIcon('bookmark', 24);
...
export function getIcon(
    ...
    result = result.replace(/width="\d+"/, `width="${size}"`);
    result = result.replace(/height="\d+"/, `height="${size}"`);
 ...
 * @example
 * ```typescript
 * const icon = getIconWithColor('bookmark', 'var(--primary-600)', 20);
 * ```
 */
